...
#include "GDEffekseer.Renderer.h"
#include "GDEffekseer.TextureLoader.h"
#include "GDEffekseer.ModelLoader.h"
#include "GDEffekseer.MaterialLoader.h"
#include "GDEffekseer.CurveLoader.h"
#include "GDEffekseer.SoundLoader.h"
#include "GDEffekseer.SoundPlayer.h"
...
EffekseerSystem::EffekseerSystem()
{
	...
	Ref<Reference> sound = GDEffekseer::ScriptNew(soundScript);
	
	m_manager = Effekseer::Manager::Create(instanceMaxCount);
#ifndef __EMSCRIPTEN__
	m_manager->LaunchWorkerThreads(2);
#endif
	m_manager->SetTextureLoader(Effekseer::MakeRefPtr<GDEffekseer::TextureLoader>());
	m_manager->SetModelLoader(Effekseer::MakeRefPtr<GDEffekseer::ModelLoader>());
	m_manager->SetMaterialLoader(Effekseer::MakeRefPtr<GDEffekseer::MaterialLoader>());
	m_manager->SetCurveLoader(Effekseer::MakeRefPtr<GDEffekseer::CurveLoader>());
	m_manager->SetSoundLoader(Effekseer::MakeRefPtr<GDEffekseer::SoundLoader>(sound));

	m_renderer = GDEffekseer::Renderer::Create(squareMaxCount, drawMaxCount);
	...
	m_manager->SetSoundPlayer(Effekseer::MakeRefPtr<GDEffekseer::SoundPlayer>(sound));
}
...
{
	Effekseer:: Matrix44 matrix = GDEffekseer::ToEfkMatrix44(camera_transform.inverse());
