...
void EffekseerEmitter2D::_process(float delta)
{
	...
	for (int i = 0; i < m_handles.size(); ) {
		...
		manager->SetMatrix(handle, GDEffekseer::ToEfkMatrix43(get_global_transform(), 
	...
	if (m_effect.is_valid()) {
		...
		if (handle >= 0) {
			...
			if (m_target_position != Vector2::ZERO) {
				...
				manager->SetTargetLocation(handle, GDEffekseer::ToEfkVector3(scaled_position));
			}
...
{
	m_color = GDEffekseer::ToEfkColor(color);
...
{
	return GDEffekseer::ToGdColor(m_color);
}
...
void EffekseerEmitter2D::set_target_position(Vector2 position)
{
	...
	for (int i = 0; i < m_handles.size(); i++) {
		manager->SetTargetLocation(m_handles[i], GDEffekseer::ToEfkVector3(scaled_position));
	}
}
